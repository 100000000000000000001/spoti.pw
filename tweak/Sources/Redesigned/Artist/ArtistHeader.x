...
// language. More is moved up into the navigation bar by a flag (ArtistField.x).
...
static char kTitleKey, kMetaKey, kShuffleKey, kPlayKey, kFollowKey, kArtworkKey, kBarKey;
...
static void applyHeader(UIView *header) {
    ...
    else if (container.subviews.lastObject != info) [container bringSubviewToFront:info];
    ...
    if (!logged && header.window && name) {
        logged = YES;
        SGLog(@"redesign artist: own block \"%@\", \"%@\"; shuffle %@, play %@, follow %@", name,
              firstText(listeners) ?: @"no listeners", shuffle ? @"found" : @"missing", play ? @"found" : @"missing",
              follow ? firstText(follow) ?: @"found" : @"missing");
    }
}
